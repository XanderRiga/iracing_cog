...
class LeaderboardDb:
    ...
    async def call(self, ctx, category, type):
        self.delete_user_guild_relationships(ctx.guild)
        async with ctx.typing():
            ...
            table_html_string = self.get_leaderboard_html_string(drivers, ctx.guild, Category.from_name(category), is_yearly)
    ...
    def get_leaderboard_html_string(self, drivers, guild, category, yearly=False):
        ...
        for index, driver in enumerate(drivers, start=1):
            try:
                if yearly:
                    stat = driver.current_year_stat(category)
                else:
                    stat = driver.career_stat(category)
                ...
                if stat:
                    table.add_row(
                        [
                            ...
                            str(driver.current_irating(category).value),
                            str(driver.peak_irating(category).value),
                            str(driver.current_license_class(category).value),
                            str(stat.total_wins),
    ...
    def delete_user_guild_relationships(self, guild):
        ...
        for driver in guild_drivers:
            if driver.discord_id not in current_member_ids:
